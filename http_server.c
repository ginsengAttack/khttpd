...
// static int should_keep_alive(struct http_request *request)
// {

//     for (size_t i = num_headers-1; i>=0; i++) {
//         if (strncmp(headers[i].name, "Connection")) {
//             if (strncmp(headers[i].value, "keep-alive"))
//                 return 1;
//             else
//                 return 0;
//         }
...
static int phr_parse(struct http_request *request,
                     char *buf,
                     ssize_t size,
                     int *keep_alive)
{
    ...
    for (int i = num_headers - 1; i >= 0; i--) {
        if (!strncasecmp(headers[i].name, "Connection",
                         (size_t) headers[i].name_len)) {
            if (!strncasecmp(headers[i].value, "keep-alive",
                             (size_t) headers[i].value_len))
                should_keep_alive = 1;
            else
                should_keep_alive = 0;
            break;
    ...
    *keep_alive = should_keep_alive;
    http_server_response(
        request, should_keep_alive);  // http_should_keep_alive(parser));
    request->complete = 1;
